# Graph Report - harness_template  (2026-07-10)

## Corpus Check
- 463 files · ~768,654 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9544 nodes · 20673 edges · 666 communities (384 shown, 282 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 392 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9b3826c5`
...
2. `el()` - 55 edges
3. `el()` - 55 edges
4. `RagSettings` - 41 edges
5. `load_rag_settings()` - 34 edges
6. `AgentState` - 32 edges
7. `runHook()` - 30 edges
8. `runHook()` - 30 edges
9. `runHook()` - 30 edges
...
- `normalize_clarification_answers()` --indirect_call--> `ClarificationAnswer`  [INFERRED]
  agent/clarification.py → graph/schemas.py
...
- `checkPageTypography()` --indirect_call--> `el()`  [INFERRED]
  .gemini/skills/impeccable/scripts/detector/detect-antipatterns-browser.js → .gemini/skills/impeccable/scripts/live-browser.js
...
- **RAG and Memory Pipeline** — rag_rewrite, rag_extract, rag_service, memory_checkpoint [INFERRED 0.85]

## Communities (666 total, 282 thin omitted)
...
Cohesion: 0.03
Nodes (133): acceptedDomAlreadyClean(), addManualContextText(), applyPlaceholderSizingStyles(), applySvelteComponentVariantStyle(), averageRgb01(), bindEditBadgeProxy(), bufferToBase64(), buildColorModels() (+125 more)
...
Cohesion: 0.05
Nodes (87): actioner_agent(), _format_score_context(), _heuristic_loop_score(), Score the loop, optionally pause for HITL skill preview, then route.      When t, Derive a loop score without calling the LLM., Build the actioner scoring prompt from harness state., Score loop quality with structured LLM output and heuristic fallback., score_loop() (+79 more)

### Community 7 - "Graph Run API Routes"
Cohesion: 0.08
Nodes (34): graph_for_checkpoint(), graph_for_request(), invoke_with_timeout(), Request, Compiled graph accessors and invocation helpers., Return the auto or step graph compiled at startup., Return the graph that owns this thread's checkpoint view., Invoke the graph with a wall-clock timeout.      ``state`` may be the initial st (+26 more)

### Community 8 - "React Graph UI Components"
Cohesion: 0.06
Nodes (72): App(), buildOverrides(), clarificationQuestions(), CommandColumn(), CommandColumnProps, parsePlanLines(), graphNodeLabel(), GraphSpine() (+64 more)
...
Cohesion: 0.04
Nodes (93): Context assembly: rewrite, memory retrieve, inject., assemble_context(), Assemble memory context for prompt injection., Rewrite query, retrieve memories, rerank, and format for injection.      Args:, Protocol, _env_bool(), _env_float(), _env_int() (+85 more)
...
Cohesion: 0.13
Nodes (27): Build initial LangGraph state from API run requests., Distill harness thread runs into reusable Cursor Agent Skills., HarnessSkillMeta, Provenance metadata stored beside each distilled skill., Summary row for GET /skills., SkillSummary, list_skills(), meta_path() (+19 more)
...
### Community 53 - "RAG Retrieval Configuration"
Cohesion: 0.09
...
Cohesion: 0.08
Nodes (35): TestClient, client(), _mock_graph(), Any, Tests for the FastAPI service., RunResponse reflects checkpoint eligibility for skill distillation., POST /resume returns 409 for non-HITL threads., POST /resume forwards overrides to update_state. (+27 more)
...
Cohesion: 0.13
Nodes (22): Eligibility rules for distilling harness threads into skills., Any, MonkeyPatch, Tests for the actioner agent node., High-scoring HITL loops pause at the actioner for skill preview., Low-scoring loops continue without a HITL pause at the actioner., Build a minimal AgentState dict for actioner tests., Actioner must audit the routing decision and score. (+14 more)
...
Cohesion: 0.29
Nodes (8): distill_skill_route(), DistillSkillRequest, DistillSkillResponse, Request, SaveSkillRequest, Distill a thread into a skill draft; set save=true to write immediately., Persist a previewed skill draft to .cursor/skills/., save_skill_route()

### Community 107 - "Task Validation & Tools"
Cohesion: 0.14
Nodes (22): Require at least one question when clarification is requested., get_cached_mcp_tools(), Return MCP tools registered at startup., Self, list_dir(), Read up to ``max_bytes`` bytes from a workspace file., List entries inside a workspace directory., read_file() (+14 more)
...
Cohesion: 0.11
Nodes (27): get_embeddings(), Any, Return an embeddings model based on RAG settings.      Args:         settings: O, chunk_documents(), ingest_documents(), load_documents(), Document, Path (+19 more)
...
Cohesion: 0.20
Nodes (12): get_mcp_manager(), init_mcp_manager(), MCP client session manager and tool invocation., Return the process-wide MCP manager when configured., Create the global MCP manager when servers are configured., Model Context Protocol (MCP) client integration for the harness., get_cached_mcp_tool(), init_mcp_tools() (+4 more)
...
Cohesion: 0.06
Nodes (54): FakeEmbeddings, ExtractedMemory, RagManifest, Pydantic schemas for RAG ingest, retrieve, and memory., One atomic memory extracted from chat history., Metadata written alongside document indexes at ingest time., memory_dir(), MemoryStore (+46 more)

### Community 133 - "MCP Tool Formatting"
Cohesion: 0.16
Nodes (15): format_call_tool_result(), qualify_tool_name(), Build a stable executor tool name for an MCP tool., Serialize an MCP CallToolResult for LLM tool messages., parse_mcp_tool_name(), Parse ``mcp__{server}__{tool}`` back into server and tool names., Tests for MCP client integration., Qualified names are stable and namespaced. (+7 more)
...
Cohesion: 0.18
Nodes (15): _llm_credentials_configured(), Any, MonkeyPatch, Tests for the reviewer agent node., Reviewer must pass execution details and tool calls into the prompt., Live smoke: reviewer returns a structured verdict for executor output., Return True when the configured provider has credentials., Build a minimal AgentState dict for reviewer tests. (+7 more)
...
Cohesion: 0.21
Nodes (12): Any, Return whether a thread has completed at least one harness loop.      A loop is, thread_eligible_for_skill(), Tests for skill distillation eligibility., Threads need executor or reviewer output from a completed loop., Threads with one round and a passing score can be distilled., Skill preview requires an actioner score of at least 80., Threads with zero rounds cannot be distilled. (+4 more)
...
Cohesion: 0.12
Nodes (24): ActionRoute, compile_with_checkpointer(), Compile the workflow with a checkpointer.      When ``human_in_the_loop`` is Tru, Finish or refine after the memorize step., route_after_action(), Tests for graph routing and builder shape., Build a minimal AgentState dict for routing tests., Approved checks should finish. (+16 more)
...
### Community 197 - "Agent Definitions & Prompts"
Cohesion: 0.20
Nodes (8): Actioner Agent, Configuration package., Load agent prompts from prompts.yaml., Executor Agent, Planner Agent, RAG Memory Extractor, RAG Query Rewriter, Reviewer Agent
...
### Community 201 - "Responsive Design"
Cohesion: 0.08
...
Cohesion: 0.15
Nodes (13): Document, Reciprocal rank fusion for hybrid retrieval., Fuse multiple ranked document lists with reciprocal rank fusion.      Args:, reciprocal_rank_fusion(), BM25Retriever, Document, Embeddings, FAISS (+5 more)
...
Cohesion: 0.05
Nodes (63): Harness run, resume, and stream routes., get_distilled_skill(), list_distilled_skills(), SkillDetail, SkillSummary, Skill library routes., List skills distilled from prior harness threads., Return a saved skill playbook for manual runs from the console. (+55 more)

### Community 250 - "UX Onboarding Patterns"
Cohesion: 0.09
Nodes (22): Assess Onboarding Needs, Context Over Ceremony, Contextual Help, Design Onboarding Experiences, Documentation & Help, Empty State Design, Feature Discovery & Adoption, Guided Tours & Walkthroughs (+14 more)

### Community 256 - "Design System Extraction"
Cohesion: 0.29
...
### Community 275 - "Responsive Design"
Cohesion: 0.08
Nodes (25): Assess Adaptation Challenge, Breakpoints: Content-Driven, Content Adaptation, Desktop Adaptation (Mobile → Desktop), Detect Input Method, Not Just Screen Size, Email Adaptation (Web → Email), Implement Adaptations, Layout Adaptation Patterns (+17 more)

### Community 276 - "live.md"
Cohesion: 0.08
Nodes (25): append-arrays, append-string, Cleanup, Consent prompt template, CSP detection (first-time only), Drift-heal warning, Exit, First-time setup (config missing or invalid) (+17 more)

### Community 277 - "document.md"
Cohesion: 0.08
Nodes (24): Component translation rules, Narrative mapping, Pitfalls, Scan mode (approach C: auto-extract, then confirm descriptive language), Schema, Seed mode, Step 1: Confirm seed mode, Step 1: Find the design assets (+16 more)

### Community 278 - "document.md"
Cohesion: 0.08
Nodes (24): Component translation rules, Narrative mapping, Pitfalls, Scan mode (approach C: auto-extract, then confirm descriptive language), Schema, Seed mode, Step 1: Confirm seed mode, Step 1: Find the design assets (+16 more)

### Community 279 - "typeset.md"
Cohesion: 0.07
...
### Community 280 - "typeset.md"
Cohesion: 0.08
Nodes (22): Amplify the Design, Assess Current State, Color Intensification, Composition Boldness, Motion & Animation, Plan Amplification, Register, Spatial Drama (+14 more)

### Community 281 - "onboard.md"
Cohesion: 0.09
Nodes (22): Assess Onboarding Needs, Context Over Ceremony, Contextual Help, Design Onboarding Experiences, Documentation & Help, Empty State Design, Feature Discovery & Adoption, Guided Tours & Walkthroughs (+14 more)

### Community 282 - "onboard.md"
Cohesion: 0.09
Nodes (22): Assess Onboarding Needs, Context Over Ceremony, Contextual Help, Design Onboarding Experiences, Documentation & Help, Empty State Design, Feature Discovery & Adoption, Guided Tours & Walkthroughs (+14 more)

### Community 283 - "test_graph.py"
Cohesion: 0.29
Nodes (7): 1. HTTP → graph → response, 2. Agent loop (nodes + helpers), 3. Executor tool loop (detail), 4. RAG read path (planner) vs write path (memorize), 5. Startup lifespan, Function Interaction Graphs, Who owns what

### Community 284 - "The Toolkit"
Cohesion: 0.10
Nodes (20): Animate complex properties, Assess What "Extraordinary" Means Here, For data-heavy interfaces, For functional UI, For performance-critical UI, For visual/marketing surfaces, Implement with Discipline, Interact with the device (+12 more)

### Community 285 - "The Toolkit"
Cohesion: 0.10
Nodes (20): Animate complex properties, Assess What "Extraordinary" Means Here, For data-heavy interfaces, For functional UI, For performance-critical UI, For visual/marketing surfaces, Implement with Discipline, Interact with the device (+12 more)
...
### Community 287 - "The Toolkit"
Cohesion: 0.10
Nodes (20): Animate complex properties, Assess What "Extraordinary" Means Here, For data-heavy interfaces, For functional UI, For performance-critical UI, For visual/marketing surfaces, Implement with Discipline, Interact with the device (+12 more)

### Community 288 - "Polish Systematically"
Cohesion: 0.10
...
### Community 289 - "animate.md"
Cohesion: 0.10
Nodes (19): Accessibility, Assess Animation Opportunities, CSS Animations, Delight Moments, Entrance Animations, Feedback & Guidance, Implement Animations, JavaScript Animation (+11 more)

### Community 290 - "Polish Systematically"
Cohesion: 0.10
Nodes (19): Clean Up, Code Quality, Color & Contrast, Content & Copy, Design System Discovery, Edge Cases & Error States, Final Verification, Forms & Inputs (+11 more)

### Community 291 - "animate.md"
Cohesion: 0.10
Nodes (19): Accessibility, Assess Animation Opportunities, CSS Animations, Delight Moments, Entrance Animations, Feedback & Guidance, Implement Animations, JavaScript Animation (+11 more)

### Community 292 - "Polish Systematically"
Cohesion: 0.10
Nodes (19): Clean Up, Code Quality, Color & Contrast, Content & Copy, Design System Discovery, Edge Cases & Error States, Final Verification, Forms & Inputs (+11 more)

### Community 293 - "Delight Techniques"
Cohesion: 0.11
...
### Community 294 - "Delight Techniques"
Cohesion: 0.11
Nodes (18): Appropriate to Context, Assess Delight Opportunities, Celebration Moments, Compound Over Time, Delight Amplifies, Never Blocks, Delight Principles, Delight Techniques, Easter Eggs & Hidden Delights (+10 more)

### Community 295 - "execution-plan.md"
Cohesion: 0.11
Nodes (15): Execution Plans, Tech Debt Tracker, Context, Decision Log, Goal, Milestones, Progress Log, Risks (+7 more)

### Community 296 - "Delight Techniques"
Cohesion: 0.11
Nodes (18): Appropriate to Context, Assess Delight Opportunities, Celebration Moments, Compound Over Time, Delight Amplifies, Never Blocks, Delight Principles, Delight Techniques, Easter Eggs & Hidden Delights (+10 more)

### Community 297 - "colorize.md"
Cohesion: 0.11
...
### Community 298 - "colorize.md"
Cohesion: 0.11
Nodes (17): Accent Color Application, Accessibility, Assess Color Opportunity, Background & Surfaces, Balance & Refinement, Borders & Accents, Cohesion, Data Visualization (+9 more)

### Community 299 - "colorize.md"
Cohesion: 0.11
Nodes (17): Accent Color Application, Accessibility, Assess Color Opportunity, Background & Surfaces, Balance & Refinement, Borders & Accents, Cohesion, Data Visualization (+9 more)

### Community 300 - "Interaction Design"
Cohesion: 0.12
Nodes (17): CSS Anchor Positioning, Destructive Actions: Undo > Confirm, Dropdown & Overlay Positioning, Fixed Positioning Fallback, Focus Rings: Do Them Right, Form Design: The Non-Obvious, Gesture Discoverability, Interaction Design (+9 more)

### Community 301 - "Enterprise LangGraph Harness — Implementation"
Cohesion: 0.11
Nodes (19): Audit Logging, Core State, Data On Disk, Developer Console, Enterprise LangGraph Harness — Implementation, Executor Tools, Glossary, Implementation Status (+11 more)

### Community 302 - "Interaction Design"
Cohesion: 0.12
Nodes (17): CSS Anchor Positioning, Destructive Actions: Undo > Confirm, Dropdown & Overlay Positioning, Fixed Positioning Fallback, Focus Rings: Do Them Right, Form Design: The Non-Obvious, Gesture Discoverability, Interaction Design (+9 more)
...
### Community 307 - "UX Writing"
Cohesion: 0.12
Nodes (16): Avoid Redundant Copy, Confirmation Dialogs: Use Sparingly, Consistency: The Terminology Problem, Don't Blame the User, Empty States Are Opportunities, Error Message Templates, Error Messages: The Formula, Form Instructions (+8 more)

### Community 308 - "Handle `generate`"
Cohesion: 0.12
Nodes (16): 1. Read the screenshot (if present), 2. Wrap the element, 3. Load the action's reference, 4. Plan three variants: identity first, then mode, then axes, 5. Apply the freeform prompt (if present), 6. Write all variants in a single edit, 7. Parameters (composition-sized, 0–4 per variant), 8. Signal done (+8 more)

### Community 309 - "Phase 1: Discovery Interview"
Cohesion: 0.12
Nodes (15): Anti-Goals, Brief Structure, Constraints, Content & Data, Design Direction, How to use the probes, Important limits, Interview cadence (+7 more)

### Community 310 - "Typography"
Cohesion: 0.12
Nodes (16): Accessibility Considerations, Anti-reflexes worth defending against, Classic Typography Principles, Fluid Type, Font Selection & Pairing, Modern Web Typography, Modular Scale & Hierarchy, OpenType Features (+8 more)

### Community 311 - "checkElementDesignSystemDOM"
Cohesion: 0.16
Nodes (20): _as_question_dicts(), ask_clarification(), normalize_clarification_answers(), Any, Pause for structured clarification when HITL is enabled.      When ``human_in_th, Normalize question models or dicts to plain dicts., Normalize a Command(resume=...) payload into answer dicts.      Args:         re, ClarificationQuestion (+12 more)

### Community 312 - "UX Writing"
Cohesion: 0.12
Nodes (16): Avoid Redundant Copy, Confirmation Dialogs: Use Sparingly, Consistency: The Terminology Problem, Don't Blame the User, Empty States Are Opportunities, Error Message Templates, Error Messages: The Formula, Form Instructions (+8 more)
...
### Community 314 - "Phase 1: Discovery Interview"
Cohesion: 0.12
Nodes (15): Anti-Goals, Brief Structure, Constraints, Content & Data, Design Direction, How to use the probes, Important limits, Interview cadence (+7 more)

### Community 315 - "Typography"
Cohesion: 0.12
Nodes (16): Accessibility Considerations, Anti-reflexes worth defending against, Classic Typography Principles, Fluid Type, Font Selection & Pairing, Modern Web Typography, Modular Scale & Hierarchy, OpenType Features (+8 more)
...
### Community 322 - "Color & Contrast"
Cohesion: 0.13
Nodes (15): Alpha Is A Design Smell, Building Functional Palettes, Color & Contrast, Color Spaces: Use OKLCH, Contrast & Accessibility, Dangerous Color Combinations, Dark Mode Is Not Inverted Light Mode, Palette Structure (+7 more)

### Community 323 - "Generate Report"
Cohesion: 0.13
Nodes (14): 1. Accessibility (A11y), 2. Performance, 3. Theming, 4. Responsive Design, 5. Anti-Patterns (CRITICAL), Anti-Patterns Verdict, Audit Health Score, Detailed Findings by Severity (+6 more)

### Community 324 - "Brand register"
Cohesion: 0.13
Nodes (14): Brand bans (on top of the shared absolute bans), Brand permissions, Brand register, Color, Font selection procedure, Imagery, Layout, Motion (+6 more)

### Community 325 - "Color & Contrast"
Cohesion: 0.13
Nodes (15): Alpha Is A Design Smell, Building Functional Palettes, Color & Contrast, Color Spaces: Use OKLCH, Contrast & Accessibility, Dangerous Color Combinations, Dark Mode Is Not Inverted Light Mode, Palette Structure (+7 more)

### Community 326 - "Init Flow"
Cohesion: 0.13
Nodes (14): Accessibility & Inclusion, Brand & Personality, Init Flow, Interview mode, not confirmation mode, Minimum viable interview, Register (ask first; it shapes everything below), Step 1: Load current state, Step 2: Explore the codebase (+6 more)

### Community 327 - "Brand register"
Cohesion: 0.13
Nodes (14): Brand bans (on top of the shared absolute bans), Brand permissions, Brand register, Color, Font selection procedure, Imagery, Layout, Motion (+6 more)

### Community 328 - "layout.md"
Cohesion: 0.14
...
### Community 330 - "layout.md"
Cohesion: 0.14
Nodes (13): Assess Current Layout, Break Card Grid Monotony, Choose the Right Layout Tool, Create Visual Rhythm, Establish a Spacing System, Improve Layout Systematically, Live-mode signature params, Manage Depth & Elevation (+5 more)

### Community 331 - "optimize.md"
Cohesion: 0.14
Nodes (13): Animation Performance, Assess Performance Issues, Core Web Vitals Optimization, Cumulative Layout Shift (CLS < 0.1), First Input Delay (FID < 100ms) / INP (< 200ms), Largest Contentful Paint (LCP < 2.5s), Loading Performance, Network Optimization (+5 more)

### Community 332 - "layout.md"
Cohesion: 0.14
Nodes (13): Assess Current Layout, Break Card Grid Monotony, Choose the Right Layout Tool, Create Visual Rhythm, Establish a Spacing System, Improve Layout Systematically, Live-mode signature params, Manage Depth & Elevation (+5 more)

### Community 333 - "optimize.md"
Cohesion: 0.14
Nodes (13): Animation Performance, Assess Performance Issues, Core Web Vitals Optimization, Cumulative Layout Shift (CLS < 0.1), First Input Delay (FID < 100ms) / INP (< 200ms), Largest Contentful Paint (LCP < 2.5s), Loading Performance, Network Optimization (+5 more)

### Community 334 - "test_rag_tools.py"
Cohesion: 0.40
Nodes (5): checkElementHeroEyebrow(), checkElementHeroEyebrowDOM(), checkHeroEyebrow(), isAccentColor(), resolveVarRefs()

### Community 335 - "Design System — Harness Console"
Cohesion: 0.15
Nodes (13): Color Strategy, Console ↔ Artifact Mapping, Design System — Harness Console, Layout, Mood, Motion, Palette (OKLCH), Product Principles (UI) (+5 more)

### Community 336 - "Frontend Guide"
Cohesion: 0.15
...
### Community 337 - "Documentation Index"
Cohesion: 0.15
...
### Community 338 - "Simplify the Design"
Cohesion: 0.17
...
### Community 339 - "Hardening Dimensions"
Cohesion: 0.17
Nodes (11): Accessibility Resilience, Assess Hardening Needs, Edge Cases & Boundary Conditions, Error Handling, Hardening Dimensions, Input Validation & Sanitization, Internationalization (i18n), Performance Resilience (+3 more)

### Community 340 - "ValueError"
Cohesion: 0.13
...
### Community 341 - "Simplify the Design"
Cohesion: 0.17
Nodes (11): Assess Current State, Code Simplification, Content Simplification, Document Removed Complexity, Information Architecture, Interaction Simplification, Layout Simplification, Plan Simplification (+3 more)

### Community 342 - "Simplify the Design"
Cohesion: 0.17
Nodes (11): Assess Current State, Code Simplification, Content Simplification, Document Removed Complexity, Information Architecture, Interaction Simplification, Layout Simplification, Plan Simplification (+3 more)

### Community 343 - "Hardening Dimensions"
Cohesion: 0.17
Nodes (11): Accessibility Resilience, Assess Hardening Needs, Edge Cases & Boundary Conditions, Error Handling, Hardening Dimensions, Input Validation & Sanitization, Internationalization (i18n), Performance Resilience (+3 more)

### Community 344 - "critique.md"
Cohesion: 0.18
...
### Community 345 - "Nielsen's 10 Heuristics"
Cohesion: 0.18
Nodes (11): 10. Help and Documentation, 1. Visibility of System Status, 2. Match Between System and Real World, 3. User Control and Freedom, 4. Consistency and Standards, 5. Error Prevention, 6. Recognition Rather Than Recall, 7. Flexibility and Efficiency of Use (+3 more)

### Community 346 - "quieter.md"
Cohesion: 0.18
...
### Community 349 - "critique.md"
Cohesion: 0.18
Nodes (10): Action Summary, Ask the User, Assessment A: Design Review, Assessment B: Detector + Browser Evidence, Assessment Orchestration, Hard Invariants, Persist the Snapshot, Purpose (+2 more)

### Community 350 - "Nielsen's 10 Heuristics"
Cohesion: 0.18
Nodes (11): 10. Help and Documentation, 1. Visibility of System Status, 2. Match Between System and Real World, 3. User Control and Freedom, 4. Consistency and Standards, 5. Error Prevention, 6. Recognition Rather Than Recall, 7. Flexibility and Efficiency of Use (+3 more)

### Community 351 - "quieter.md"
Cohesion: 0.18
Nodes (10): Assess Current State, Color Refinement, Composition Refinement, Motion Reduction, Plan Refinement, Refine the Design, Register, Simplification (+2 more)

### Community 352 - "General rules"
Cohesion: 0.18
Nodes (11): Absolute bans, Color, Color & Theme, Design guidance, General rules, Interaction, Layout, Motion (+3 more)

### Community 353 - "critique.md"
Cohesion: 0.18
Nodes (10): Action Summary, Ask the User, Assessment A: Design Review, Assessment B: Detector + Browser Evidence, Assessment Orchestration, Hard Invariants, Persist the Snapshot, Purpose (+2 more)

### Community 354 - "Nielsen's 10 Heuristics"
Cohesion: 0.18
Nodes (11): 10. Help and Documentation, 1. Visibility of System Status, 2. Match Between System and Real World, 3. User Control and Freedom, 4. Consistency and Standards, 5. Error Prevention, 6. Recognition Rather Than Recall, 7. Flexibility and Efficiency of Use (+3 more)

### Community 355 - "quieter.md"
Cohesion: 0.18
Nodes (10): Assess Current State, Color Refinement, Composition Refinement, Motion Reduction, Plan Refinement, Refine the Design, Register, Simplification (+2 more)

### Community 356 - "General rules"
Cohesion: 0.18
Nodes (11): Absolute bans, Color, Color & Theme, Design guidance, General rules, Interaction, Layout, Motion (+3 more)

### Community 357 - "Craft Flow"
Cohesion: 0.20
Nodes (10): Craft Flow, Gates: do not compress, Production bar, Step 0: Project Foundation, Step 1: Shape the Design, Step 2: Load References, Step 3: Visual Direction & Assets (Harness-Gated), Step 4: Build to Production Quality (+2 more)

### Community 358 - "Generate Combined Critique Report"
Cohesion: 0.20
...
### Community 359 - "Product register"
Cohesion: 0.20
Nodes (9): Color, Components, Layout, Motion, Product bans (on top of the shared absolute bans), Product permissions, Product register, The product slop test (+1 more)

### Community 360 - "syncPageChatFocus"
Cohesion: 0.29
...
### Community 361 - "Product register"
Cohesion: 0.20
Nodes (9): Color, Components, Layout, Motion, Product bans (on top of the shared absolute bans), Product permissions, Product register, The product slop test (+1 more)

### Community 362 - "syncEditBadgeHitProxies"
Cohesion: 0.29
Nodes (14): attachSteerFocusDebug(), attachSteerFocusGuard(), clearSteerFocusRecoverTimer(), focusConfigureInput(), focusSteerChat(), notePagePointerDown(), pageHasHostTextSelection(), scheduleSteerFocusRecover() (+6 more)

### Community 363 - "Architecture"
Cohesion: 0.20
...
### Community 366 - "Craft Flow"
Cohesion: 0.20
Nodes (10): Craft Flow, Gates: do not compress, Production bar, Step 0: Project Foundation, Step 1: Shape the Design, Step 2: Load References, Step 3: Visual Direction & Assets (Harness-Gated), Step 4: Build to Production Quality (+2 more)

### Community 367 - "Common Cognitive Load Violations"
Cohesion: 0.22
...
### Community 371 - "Common Cognitive Load Violations"
Cohesion: 0.22
Nodes (9): 1. The Wall of Options, 2. The Memory Bridge, 3. The Hidden Navigation, 4. The Jargon Barrier, 5. The Visual Noise Floor, 6. The Inconsistent Pattern, 7. The Multi-Task Demand, 8. The Context Switch (+1 more)

### Community 372 - "Generate Combined Critique Report"
Cohesion: 0.22
Nodes (9): Anti-Patterns Verdict, Design Health Score, Generate Combined Critique Report, Minor Observations, Overall Impression, Persona Red Flags, Priority Issues, Questions to Consider (+1 more)

### Community 373 - "format_thread_context"
Cohesion: 0.23
...
### Community 374 - "Persona-Based Design Testing"
Cohesion: 0.25
Nodes (8): 1. Impatient Power User: "Alex", 2. Confused First-Timer: "Jordan", 3. Accessibility-Dependent User: "Sam", 4. Deliberate Stress Tester: "Riley", 5. Distracted Mobile User: "Casey", Persona-Based Design Testing, Project-Specific Personas, Selecting Personas

### Community 375 - "Persona-Based Design Testing"
Cohesion: 0.25
Nodes (8): 1. Impatient Power User: "Alex", 2. Confused First-Timer: "Jordan", 3. Accessibility-Dependent User: "Sam", 4. Deliberate Stress Tester: "Riley", 5. Distracted Mobile User: "Casey", Persona-Based Design Testing, Project-Specific Personas, Selecting Personas

### Community 376 - "Asyncio In This Harness"
Cohesion: 0.25
...
### Community 377 - "index.md"
Cohesion: 0.25
Nodes (6): Agent-First Operating Principles, Core Beliefs, Harness-Specific Beliefs, Conventions, Design Docs Index, Documents

### Community 378 - "Product"
Cohesion: 0.25
Nodes (8): Accessibility & Inclusion, Anti-references, Brand Personality, Design Principles, Product, Product Purpose, Register, Users

### Community 379 - "Product Sense"
Cohesion: 0.25
...
### Community 380 - "Repository Collaboration Guide"
Cohesion: 0.25
...
### Community 381 - "Persona-Based Design Testing"
Cohesion: 0.25
Nodes (8): 1. Impatient Power User: "Alex", 2. Confused First-Timer: "Jordan", 3. Accessibility-Dependent User: "Sam", 4. Deliberate Stress Tester: "Riley", 5. Distracted Mobile User: "Casey", Persona-Based Design Testing, Project-Specific Personas, Selecting Personas

### Community 382 - "Enterprise LangGraph Harness"
Cohesion: 0.25
...
### Community 384 - "Cognitive Load Assessment"
Cohesion: 0.29
Nodes (7): Cognitive Load Assessment, Cognitive Load Checklist, Extraneous Load: Bad Design, Germane Load: Learning Effort, Intrinsic Load: The Task Itself, The Working Memory Rule, Three Types of Cognitive Load

### Community 385 - "$impeccable hooks"
Cohesion: 0.10
Nodes (18): Extract Flow, Step 1: Discover the Design System, Step 2: Identify Patterns, Step 3: Plan Extraction, Step 4: Extract & Enrich, Step 5: Migrate, Step 6: Document, Constraints (+10 more)

### Community 386 - "Cognitive Load Assessment"
Cohesion: 0.29
Nodes (7): Cognitive Load Assessment, Cognitive Load Checklist, Extraneous Load: Bad Design, Germane Load: Learning Effort, Intrinsic Load: The Task Itself, The Working Memory Rule, Three Types of Cognitive Load

### Community 387 - "[YYYY-MM-DD HH:mm] | Task:"
Cohesion: 0.29
Nodes (6): Changes Overview, Design Intent, Execution Context, Files Modified, User Query, [YYYY-MM-DD HH:mm] | Task:

### Community 388 - "Architecture"
Cohesion: 0.29
Nodes (7): Agent loop, App startup (lifespan), Architecture, Graph builder, How `/run` vs `/resume` vs `/stream` differ, How status is derived, Human-in-the-loop (HITL)

### Community 389 - "HTTP API"
Cohesion: 0.29
Nodes (7): `GET /health`, HTTP API, `POST /resume`, `POST /run`, `POST /stream`, Skill lifecycle, Skills API

### Community 390 - "LangGraph Checkpoints"
Cohesion: 0.16
Nodes (10): Backends, Checkpoint Contents, Default (SQLite), LangGraph Checkpoints, Migrations, Troubleshooting, Two Compiled Graphs, Config (+2 more)

### Community 391 - "Supply Chain Security"
Cohesion: 0.29
...
### Community 392 - "Cognitive Load Assessment"
Cohesion: 0.29
Nodes (7): Cognitive Load Assessment, Cognitive Load Checklist, Extraneous Load: Bad Design, Germane Load: Learning Effort, Intrinsic Load: The Task Itself, The Working Memory Rule, Three Types of Cognitive Load

### Community 393 - "[2026-07-10 19:03] | Task: Improve planner system prompt"
Cohesion: 0.25
...
### Community 394 - "[2026-07-07 00:20] | Task: Add README, AGENTS.md, LICENSE, CONTRIBUTING, and upstream docs scaffold"
Cohesion: 0.33
...
### Community 401 - "[2026-07-10 17:55] | Task: Expand IMPLEMENTATION.md"
Cohesion: 0.33
Nodes (5): [2026-07-10 17:55] | Task: Expand IMPLEMENTATION.md, Changes Overview, Design Intent, Files Modified, User Query

### Community 402 - "Change History Guide"
Cohesion: 0.33
Nodes (6): Change History Guide, Layout And Naming, Redaction Rules, Related, Requirements, What To Include

### Community 403 - "Agent Nodes"
Cohesion: 0.33
Nodes (6): Actioner, Agent Nodes, Executor (two phases), Memorize, Planner, Reviewer

### Community 404 - "harness-template"
Cohesion: 0.40
...
### Community 405 - "Amplify the Design"
Cohesion: 0.29
...
### Community 406 - "Harness Frontend"
Cohesion: 0.40
Nodes (4): Build, Harness Frontend, Keyboard, Run locally

### Community 407 - "Structured Schemas"
Cohesion: 0.40
Nodes (5): ActionScoreResult, ExecutorResult, PlanResult, ReviewResult, Structured Schemas

### Community 408 - "RAG"
Cohesion: 0.40
Nodes (5): Doc search (executor tool), RAG, RAG configuration, Read path (planner), Write path (memorize)

### Community 409 - "Quality Score"
Cohesion: 0.40
Nodes (5): Current Scores, Example — when to bump a score, Quality Score, Related, Scale

### Community 410 - "References"
Cohesion: 0.40
Nodes (5): Avoid, External References (starting points), Good Candidates To Add Here, In-Repo Technical References, References

### Community 411 - "Heuristics Scoring Guide"
Cohesion: 0.50
Nodes (4): Heuristics Scoring Guide, Issue Severity (P0–P3), Reference Material, Score Summary

### Community 412 - "Heuristics Scoring Guide"
Cohesion: 0.50
Nodes (4): Heuristics Scoring Guide, Issue Severity (P0–P3), Reference Material, Score Summary

### Community 413 - "Configuration"
Cohesion: 0.50
Nodes (4): Checkpoints, Configuration, Frontend (console), LLM

### Community 414 - "Heuristics Scoring Guide"
Cohesion: 0.50
Nodes (4): Heuristics Scoring Guide, Issue Severity (P0–P3), Reference Material, Score Summary

### Community 415 - "[2026-07-10 20:06] | Task: HITL clarification questions"
Cohesion: 0.29
...
### Community 416 - "Running Locally"
Cohesion: 0.67
Nodes (3): Full curl walkthrough, Running Locally, Start stack

### Community 417 - "feature-release-notes.md"
Cohesion: 0.40
Nodes (5): checkElementHeroEyebrow(), checkElementHeroEyebrowDOM(), checkHeroEyebrow(), isAccentColor(), resolveVarRefs()

### Community 418 - "MCP integration for the harness"
Cohesion: 0.50
Nodes (5): format_rewrite_input(), BaseMessage, Format task and recent messages for query rewrite.      Args:         task: Curr, Rewrite a task into memory search queries.      Args:         task: Current user, rewrite_for_memory()

### Community 419 - "health.py"
Cohesion: 0.50
...
## Knowledge Gaps
- **2279 isolated node(s):** `COMMON_DEV_PORTS`, `SCANNABLE_EXT`, `SOURCE_DIRS`, `PRODUCT_NAMES`, `DESIGN_NAMES` (+2274 more)
...
- **Why does `el()` connect `Configuration Bar UI Controls` to `live-browser.js`, `Visual Contrast Analysis`, `css-cascade.mjs`, `design-system.mjs`, `detect-html.mjs`, `checks.mjs`, `Browser Visual Contrast Analysis`, `parseAnyColor`, `Page Chat Voice UI`, `Live Editing State Management`, `Browser Finding Serialization`, `collectBrowserFindings`, `resolveLengthPx`, `detect-antipatterns-browser.js`, `Parameter Panel UI`, `UI Toolbar & Themes`, `detect-antipatterns.mjs`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `el()` connect `el` to `Visual Contrast Element Checks`, `CSS Cascade Style Parsing`, `Design System Token Management`, `Static Content Style Analysis`, `Design System Validation Checks`, `Browser Style Validation`, `live-browser.js`, `Parameter Panel UI`, `Browser Visual Contrast Analysis`, `Browser Design Analysis`, `Browser Finding Serialization`, `UI Theme Design Panel`, `Page Chat State Management`, `Element Quality Checks`, `Browser Antipattern Detection`, `Antipattern Detection CLI`, `Live Session Cleanup State`?**
