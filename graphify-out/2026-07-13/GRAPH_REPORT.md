...
## Corpus Check
- 499 files · ~792,008 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9890 nodes · 21189 edges · 691 communities (404 shown, 287 thin omitted)
...
- `test_learner_writes_learning_to_state()` --indirect_call--> `LearningResult`  [INFERRED]
  tests/test_learner.py → graph/schemas.py
...
## Communities (691 total, 287 thin omitted)
...
Cohesion: 0.04
Nodes (82): ActionRoute, actioner_agent(), _format_score_context(), _heuristic_loop_score(), merge_learning_and_extract(), _normalize_step(), Score loop quality with structured LLM output and heuristic fallback., Soft-skip on fail; on pass score, HITL, and commit memories.      Human-in-the-l (+74 more)

### Community 7 - "Graph Run API Routes"
Cohesion: 0.05
Nodes (57): _format_tool_calls(), learner_agent(), ToolCallRecord, Format compact tool call records for the learner prompt.      Args:         tool, Challenge the current result and return structured learning., planner_agent(), Draft or revise the implementation plan., Configuration package. (+49 more)
...
Cohesion: 0.07
Nodes (43): get_distilled_skill(), list_distilled_skills(), SkillDetail, SkillSummary, Skill library routes., List skills distilled from prior harness threads., Return a saved skill playbook for manual runs from the console., Pydantic request and response models for the HTTP API. (+35 more)
...
Cohesion: 0.03
Nodes (115): Memory commit helpers used by the actioner after approval., Protocol, _env_bool(), _env_float(), _env_int(), ExtractSettings, HybridSettings, InjectSettings (+107 more)
...
Cohesion: 0.08
Nodes (51): detectUrl(), runVisualContrastFallback(), serializeDesignSystemForBrowser(), CSS_IN_JS_EXTENSIONS, detectText(), extFromFilePath(), extractCSSinJS(), extractStyleBlocks() (+43 more)
...
Cohesion: 0.06
Nodes (30): Two-stage rerank settings., RerankSettings, BiEncoderReranker, _cosine(), Document, Embeddings, Stage-1 bi-encoder reranker using embedding cosine similarity., Rerank documents with bi-encoder cosine similarity. (+22 more)

### Community 85 - "Design Document Parser"
Cohesion: 0.06
Nodes (57): FakeEmbeddings, chunk_documents(), ingest_documents(), load_documents(), Document, Path, Static document ingest pipeline., Load text documents from a directory.      Args:         source_dir: Directory c (+49 more)
...
Cohesion: 0.07
Nodes (42): Response body for graph execution and resume., RunResponse, RunResponse, Translate the latest checkpoint into an API response., snapshot_to_response(), client(), _mock_graph(), Any (+34 more)
...
Cohesion: 0.07
Nodes (42): Request, ResumeRequest, RunRequest, RunResponse, Harness run, resume, and stream routes., Start or continue a graph run for the given thread., Resume a human-in-the-loop thread., Stream graph updates for a run (Server-Sent Events). (+34 more)
...
Cohesion: 0.15
Nodes (13): _build_langchain_tool(), _json_schema_to_fields(), Any, Map a small JSON-schema object to Pydantic fields for StructuredTool., Wrap one MCP tool as a LangChain StructuredTool., StructuredTool, MonkeyPatch, LangChain MCP tools call through the shared manager. (+5 more)
...
Cohesion: 0.10
Nodes (32): get_mcp_manager(), init_mcp_manager(), McpClientManager, qualify_tool_name(), MCP client session manager and tool invocation., List tools from every connected MCP server., Return the process-wide MCP manager when configured., Create the global MCP manager when servers are configured. (+24 more)
...
Cohesion: 0.22
Nodes (8): format_call_tool_result(), Any, Invoke a tool on a connected MCP server., Serialize an MCP CallToolResult for LLM tool messages., Text MCP content is flattened for executor tool messages., MCP errors are prefixed for the executor., test_format_call_tool_result_error(), test_format_call_tool_result_text()
...
Cohesion: 0.13
Nodes (22): LessonsBlock, Learner lessons attached for actioner scoring and UI., _llm_credentials_configured(), Any, MonkeyPatch, Tests for the learner agent node., Learner must pass execution details and tool calls into the prompt., Return True when the configured provider has credentials. (+14 more)
...
Cohesion: 0.12
Nodes (18): _interrupt_payload(), Any, Map LangGraph checkpoints to API responses., Build InterruptPayload from the first active snapshot interrupt., InterruptPayload, Any, Eligibility rules for distilling harness threads into skills., Return whether a thread has completed at least one harness loop.      A loop is (+10 more)
...
Cohesion: 0.14
Nodes (18): distill_skill_route(), DistillSkillRequest, DistillSkillResponse, Request, SaveSkillRequest, Distill a thread into a skill draft; set save=true to write immediately., Persist a previewed skill draft to .cursor/skills/., save_skill_route() (+10 more)
...
Cohesion: 0.14
Nodes (11): PostgresMemoryStore, Document, Embeddings, Path, Postgres + pgvector memory store for extracted chat memories., Format a float list as a pgvector literal., Thread-scoped memory store backed by pgvector., No-op — Postgres is the source of truth. (+3 more)
...
Cohesion: 0.07
Nodes (66): Business logic for the harness skill library API., _build_user_prompt(), distill_skill_from_thread(), _draft_skill(), Any, DistillSkillResponse, Distill a harness thread checkpoint into a reusable Cursor skill., Persist a previewed skill draft without re-running the LLM. (+58 more)
...
## Knowledge Gaps
- **2438 isolated node(s):** `COMMON_DEV_PORTS`, `SCANNABLE_EXT`, `SOURCE_DIRS`, `PRODUCT_NAMES`, `DESIGN_NAMES` (+2433 more)
...
- **Why does `el()` connect `Configuration Bar UI Controls` to `live-browser.js`, `UI Toolbar & Themes`, `Element Quality Checks`, `Visual Contrast Analysis`, `Static CSS Processing`, `Design System Token Management`, `Static Content Style Analysis`, `Design System Validation Checks`, `Browser Visual Contrast Analysis`, `Browser Style Validation`, `Browser Antipattern Detection`, `initPageChat`, `Browser Finding Serialization`, `Live Editing State Management`, `Browser Design Analysis`, `Parameter Panel UI`?**
