# Graph Report - harness_template  (2026-07-14)

## Corpus Check
- 522 files · ~807,270 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10185 nodes · 21703 edges · 700 communities (417 shown, 283 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 390 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9ccf0d75`
...
- `create_workflow()` --indirect_call--> `actioner_agent()`  [INFERRED]
  graph/builder.py → agent/actioner.py
...
## Communities (700 total, 283 thin omitted)
...
Cohesion: 0.03
Nodes (136): acceptedDomAlreadyClean(), applyPlaceholderSizingStyles(), applySvelteComponentVariantStyle(), attachSteerFocusGuard(), averageRgb01(), bufferToBase64(), buildCollapsible(), buildColorModels() (+128 more)

### Community 2 - "live-browser.js"
Cohesion: 0.03
Nodes (139): addManualContextText(), applyGlobalBarLabelState(), applySvelteComponentVariantStyle(), averageRgb01(), bufferToBase64(), buildCollapsible(), buildColorModels(), buildInsertPlaceholderSnapshotFromDom() (+131 more)
...
Cohesion: 0.06
Nodes (55): ActionRoute, _format_score_context(), _heuristic_loop_score(), _normalize_step(), Score loop quality with structured LLM output and heuristic fallback., Map refine/suggested steps to planner or finish., Derive a loop score without calling the LLM., Build the actioner scoring prompt from harness state. (+47 more)
...
Cohesion: 0.08
Nodes (85): abortSvelteComponentInjection(), applySavedSessionMeta(), cancelEditing(), cancelEditingToPicking(), cancelInsertConfigure(), cleanup(), cleanupAcceptedSession(), clearAnnotations() (+77 more)
...
Cohesion: 0.06
Nodes (45): health(), Request, Return service health., distill_skill_route(), get_distilled_skill(), list_distilled_skills(), DistillSkillRequest, DistillSkillResponse (+37 more)
...
Cohesion: 0.09
Nodes (44): _extract_round_sections(), _format_value(), _index_path(), init_thread_artifacts(), lookup_thread_dir(), _pending_stage_markdown(), Any, Path (+36 more)

### Community 26 - "el"
Cohesion: 0.07
Nodes (56): checkPageTypography(), checkPageTypography(), actionLabel(), applyConfigureBarChrome(), bindConfigureCountPillTooltip(), bindConfigureInlineControlHover(), bindConfigureModifierPillHover(), buildConfigureActionControl() (+48 more)

### Community 27 - "RAG Memory Schemas Extraction"
Cohesion: 0.04
Nodes (105): Assemble memory context for prompt injection., FakeEmbeddings, _env_bool(), _env_float(), _env_int(), ExtractSettings, HybridSettings, InjectSettings (+97 more)
...
Cohesion: 0.10
Nodes (36): applyParamDefaults(), applyParamValue(), barPaletteForTheme(), brandMarkSvg(), buildDesignHeader(), buildParamsPanel(), designPanelCss(), detectPageTheme() (+28 more)
...
Cohesion: 0.13
Nodes (19): FastAPI lifespan: checkpointer, pools, and compiled LangGraph instances., Close the global MCP manager., shutdown_mcp_manager(), _compile_graphs(), Checkpoint backend selection for local dev and production., Return the filesystem path or URI for the SQLite checkpointer., sqlite_checkpoint_path(), graph_lifespan() (+11 more)
...
Cohesion: 0.08
Nodes (36): Path, Tests for scripts/manifest.json loading., Missing manifest yields an empty entry list., Valid manifest entries are parsed into ManifestEntry objects., require_entry raises when path is not listed., test_load_manifest_empty_when_missing(), test_load_manifest_parses_entries(), test_require_entry_rejects_unknown() (+28 more)
...
Cohesion: 0.16
Nodes (21): RagManifest, Metadata written alongside document indexes at ingest time., build_vectorstore(), faiss_dir(), load_manifest(), load_vectorstore(), manifest_path(), Document (+13 more)
...
Cohesion: 0.08
Nodes (37): client(), _mock_graph(), Any, TestClient, Tests for the FastAPI service., RunResponse reflects checkpoint eligibility for skill distillation., RunResponse exposes dynamic interrupt and compact learning fields., POST /resume returns 409 for non-HITL threads. (+29 more)
...
Cohesion: 0.09
Nodes (42): resolveProjectRoot(), firstExisting(), getDesignSidecarCandidates(), getDesignSidecarPath(), getImpeccableDir(), getLegacyLiveAnnotationsDir(), getLegacyLiveConfigPath(), getLegacyLiveServerPath() (+34 more)
...
Cohesion: 0.12
Nodes (27): applyStaticDeclaration(), buildBorderOverrideMap(), buildStaticWindow(), collectStaticCssRules(), compareStaticPriority(), cssPropToCamel(), expandStaticBoxValues(), expandStaticDeclaration() (+19 more)
...
Cohesion: 0.05
Nodes (73): Harness run, resume, and stream routes., graph_for_request(), invoke_with_timeout(), Compiled graph accessors and invocation helpers., Return the auto or step graph compiled at startup., Invoke the graph with a wall-clock timeout., Database and graph persistence layer for the API., Pydantic request and response models for the HTTP API. (+65 more)
...
Cohesion: 0.14
Nodes (18): applyEditing(), buildLocatorForLeaf(), buildPlaceholderResizeHandles(), copyEditContainerContext(), copyEditLeafContext(), cursorForPlaceholderEdge(), documentRefForElement(), extractContext() (+10 more)
...
Cohesion: 0.03
Nodes (96): Protocol, Two-stage rerank settings., RerankSettings, RAG subsystem: ingest, retrieve, rerank, and inject., BiEncoderReranker, _cosine(), Document, Embeddings (+88 more)
...
Cohesion: 0.15
Nodes (24): getLiveAnnotationsDir(), getLiveDir(), scrubManualEditsAgainstFile(), scrubManualEditsAgainstOriginalBlock(), args, buffer, cwd, pageUrlFilter (+16 more)
...
Cohesion: 0.07
Nodes (27): Assess Current State, Plan Amplification, Register, Verify Quality, Extract Flow, Step 1: Discover the Design System, Step 2: Identify Patterns, Step 3: Plan Extraction (+19 more)
...
Cohesion: 0.14
Nodes (17): format_call_tool_result(), qualify_tool_name(), Build a stable executor tool name for an MCP tool., Serialize an MCP CallToolResult for LLM tool messages., parse_mcp_tool_name(), Parse ``mcp__{server}__{tool}`` back into server and tool names., Tests for MCP client integration., Qualified names are stable and namespaced. (+9 more)
...
Cohesion: 0.16
Nodes (15): load_mcp_server_configs(), _parse_servers_payload(), Load MCP server definitions from env or config files., Normalize JSON into server configs., Load MCP server configs when MCP is enabled., McpServersDocument, McpStdioServerConfig, Schemas for MCP server configuration. (+7 more)
...
Cohesion: 0.13
Nodes (14): get_mcp_manager(), init_mcp_manager(), McpClientManager, Any, MCP client session manager and tool invocation., List tools from every connected MCP server., Invoke a tool on a connected MCP server., Return the process-wide MCP manager when configured. (+6 more)
...
Cohesion: 0.08
Nodes (40): get_cached_mcp_tools(), Return MCP tools registered at startup., MonkeyPatch, Path, Tests for executor tool registry defaults and thread binding., Default EXECUTOR_TOOLS includes write/read thread file tools., get_executor_tools(thread_id) returns bound thread file tools., test_default_allowlist_includes_thread_file_tools() (+32 more)
...
Cohesion: 0.10
Nodes (35): LearningCandidate, LearningResult, LessonsBlock, Learner lessons attached for actioner scoring and UI., Memory-shaped candidate proposed by the learner., Structured learner output consumed by the actioner., _llm_credentials_configured(), Any (+27 more)
...
Cohesion: 0.11
Nodes (15): Execution Plans, Tech Debt Tracker, Context, Decision Log, Goal, Milestones, Progress Log, Risks (+7 more)
...
Cohesion: 0.11
Nodes (18): Action-end memory review (HITL), Action review panel, Actioner order of operations, Audit, Decisions, Docs to update (implementation task), Draft state, Errors (+10 more)
...
Cohesion: 0.06
Nodes (70): actioner_agent(), merge_learning_and_extract(), Soft-skip on fail; on pass score, HITL, and commit memories.      Human-in-the-l, Dedupe by normalized content and assign stable ids m0.., action_review_message(), clear_pending(), load_pending(), load_score() (+62 more)
...
Cohesion: 0.17
Nodes (17): CheckpointBackend, Return the configured LangGraph checkpoint backend.      Priority:     1. ``CHEC, resolve_checkpoint_backend(), MonkeyPatch, Tests for checkpoint backend selection., Local dev defaults to SQLite when backend is not explicit., CHECKPOINT_BACKEND=memory overrides defaults., Compose-style DATABASE_URL alone should not pick Postgres locally. (+9 more)
...
Cohesion: 0.15
Nodes (16): McpToolDescriptor, One tool exposed by an MCP server., _build_langchain_tool(), get_cached_mcp_tool(), init_mcp_tools(), _json_schema_to_fields(), Any, BaseTool (+8 more)
...
Cohesion: 0.10
Nodes (22): Require a task and/or a skill slug., list_dir(), ListDirInput, Read up to ``max_bytes`` bytes from a workspace file., Input schema for the list_dir tool., List entries inside a workspace directory., Input schema for the read_file tool., read_file() (+14 more)
...
Cohesion: 0.39
Nodes (6): CommandColumn(), CommandColumnProps, useNarrowViewport(), DistillSkillResponse, SkillDetail, SkillSummary
...
Cohesion: 0.40
Nodes (5): Interrupt value, Pending / approved item (state + interrupt), Resume mapping rules, Resume value (`Command(resume=…)`), Schemas
...
Cohesion: 0.04
Nodes (71): executor_agent(), _format_tool_evidence(), Any, BaseMessage, ToolCallRecord, Run a bounded tool-calling loop, then summarize as ExecutorResult.      Phase 1:, Serialize a tool return value to a ToolMessage content string., Render tool-loop history as plain text for the summarize phase.      Live ``Tool (+63 more)
...
Cohesion: 0.05
Nodes (84): get_skill_detail(), SkillDetail, Business logic for the harness skill library API., Return a saved skill playbook., _format_messages(), format_thread_context(), Any, BaseMessage (+76 more)
...
## Knowledge Gaps
- **2505 isolated node(s):** `COMMON_DEV_PORTS`, `SCANNABLE_EXT`, `SOURCE_DIRS`, `PRODUCT_NAMES`, `DESIGN_NAMES` (+2500 more)
...
- **Why does `el()` connect `el` to `Visual Contrast Element Checks`, `CSS Cascade Style Parsing`, `Design System Token Management`, `Static Content Style Analysis`, `Design System Validation Checks`, `Browser Style Validation`, `live-browser.js`, `Browser Visual Contrast Analysis`, `documentRefForElement`, `Browser Design Analysis`, `Browser Finding Serialization`, `resumeSession`, `test_planner.py`, `UI Theme Design Panel`, `Page Chat State Management`, `Element Quality Checks`, `Browser Antipattern Detection`, `Manual Edit UI State`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
