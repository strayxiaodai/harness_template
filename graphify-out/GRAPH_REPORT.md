...
## Corpus Check
- 534 files · ~814,599 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10305 nodes · 21876 edges · 699 communities (416 shown, 283 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 390 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `84cdd9f8`
...
## Communities (699 total, 283 thin omitted)
...
Cohesion: 0.05
Nodes (68): get_distilled_skill(), list_distilled_skills(), SkillDetail, SkillSummary, Skill library routes., List skills distilled from prior harness threads., Return a saved skill playbook for manual runs from the console., Pydantic request and response models for the HTTP API. (+60 more)
...
Cohesion: 0.05
Nodes (64): list_thread_artifacts(), Thread artifact library routes., List on-disk thread artifacts under app/threads/ for console attach., Schemas for thread artifact list endpoints., Summary row for GET /threads., ThreadSummary, _extract_round_sections(), _format_value() (+56 more)
...
Cohesion: 0.05
Nodes (72): FakeEmbeddings, InjectSettings, RagSettings, Top-level RAG configuration., Prompt injection settings., collect_new_messages(), commit_approved_memories(), extract_memories() (+64 more)
...
Cohesion: 0.06
Nodes (50): FastAPI lifespan: checkpointer, pools, and compiled LangGraph instances., Any, Install a pooled connection used by all audit writes.      The pool is owned by, Write one audit row when a pool is configured.      Args:         thread_id: Lan, set_audit_pool(), write_audit_event(), CheckpointBackend, Close the global MCP manager. (+42 more)
...
Cohesion: 0.05
Nodes (65): chunk_documents(), ingest_documents(), load_documents(), Document, Path, Load text documents from a directory.      Args:         source_dir: Directory c, Split documents into chunks with stable ids.      Args:         documents: Sourc, Ingest static documents into FAISS and BM25 indexes.      Args:         source_d (+57 more)
...
Cohesion: 0.05
Nodes (72): Request, ResumeRequest, RunRequest, RunResponse, Harness run, resume, and stream routes., Start or continue a graph run for the given thread., Resume a human-in-the-loop thread., Stream graph updates for a run (Server-Sent Events). (+64 more)
...
Cohesion: 0.11
Nodes (31): _as_question_dicts(), ask_clarification(), format_clarification_block(), merge_clarification_answers(), normalize_clarification_answers(), Any, HITL clarification helper shared by harness agent nodes., Pause for structured clarification when HITL is enabled.      When ``human_in_th (+23 more)
...
Cohesion: 0.08
Nodes (20): After This File, Codex: Visual Direction & Asset Production, Four stop points before code, Step A: Explore Directions with the User, Step B: Generate the Brand Palette First, Step C: Generate 1-3 Visual Mocks Against the Palette, Step D: Approval Loop, Step E: Mock Fidelity Inventory (+12 more)

### Community 166 - "Context Signal Extraction"
Cohesion: 0.12
Nodes (21): distill_skill_route(), DistillSkillRequest, DistillSkillResponse, Request, SaveSkillRequest, Distill a thread into a skill draft; set save=true to write immediately., Persist a previewed skill draft to app/skills/., save_skill_route() (+13 more)
...
Cohesion: 0.03
Nodes (84): Context assembly: rewrite, memory retrieve, inject., assemble_context(), Assemble memory context for prompt injection., Rewrite query, retrieve memories, rerank, and format for injection.      Args:, _env_bool(), _env_float(), _env_int(), ExtractSettings (+76 more)
...
Cohesion: 0.09
Nodes (25): Require a task and/or a skill slug., list_dir(), ListDirInput, Read up to ``max_bytes`` bytes from a workspace file., Input schema for the list_dir tool., List entries inside a workspace directory., Input schema for the read_file tool., read_file() (+17 more)
...
Cohesion: 0.10
Nodes (33): LearningResult, LessonsBlock, Learner lessons attached for actioner scoring and UI., Structured learner output consumed by the actioner., _llm_credentials_configured(), Any, MonkeyPatch, Tests for the learner agent node. (+25 more)
...
Cohesion: 0.05
Nodes (73): actioner_agent(), merge_learning_and_extract(), _normalize_step(), Soft-skip on fail; on pass score, HITL, and commit memories.      Human-in-the-l, Map refine/suggested steps to planner or finish., Dedupe by normalized content and assign stable ids m0.., action_review_message(), clear_pending() (+65 more)
...
Cohesion: 0.08
Nodes (38): Path, Tests for scripts/manifest.json loading., Missing manifest yields an empty entry list., Valid manifest entries are parsed into ManifestEntry objects., require_entry raises when path is not listed., test_load_manifest_empty_when_missing(), test_load_manifest_parses_entries(), test_require_entry_rejects_unknown() (+30 more)
...
Cohesion: 0.04
Nodes (86): format_call_tool_result(), get_mcp_manager(), init_mcp_manager(), McpClientManager, Any, qualify_tool_name(), MCP client session manager and tool invocation., List tools from every connected MCP server. (+78 more)
...
Cohesion: 0.03
Nodes (110): ActionRoute, _format_score_context(), _heuristic_loop_score(), Score loop quality with structured LLM output and heuristic fallback., Derive a loop score without calling the LLM., Build the actioner scoring prompt from harness state., score_loop(), executor_agent() (+102 more)
...
Cohesion: 0.07
Nodes (49): _format_messages(), format_thread_context(), Any, BaseMessage, Serialize harness thread state for skill distillation., Build a readable snapshot of a completed harness thread.      Args:         valu, _build_user_prompt(), distill_skill_from_thread() (+41 more)
...
Cohesion: 0.28
Nodes (8): Any, Unit tests for llm.providers.get_llm., vllm provider should construct ChatOpenAI against VLLM_* env., When VLLM_API_KEY is unset, api_key should be EMPTY., VLLM_MAX_TOKENS and VLLM_ENABLE_THINKING should pass through., test_get_llm_vllm_defaults_api_key_to_empty(), test_get_llm_vllm_respects_max_tokens_and_thinking(), test_get_llm_vllm_uses_openai_compatible_client()
...
- **Why does `el()` connect `Configuration Bar UI Controls` to `live-browser.js`, `UI Toolbar & Themes`, `Design System Validation Checks`, `Browser Visual Contrast Analysis`, `Browser Style Validation`, `Live Editing State Management`, `Parameter Panel UI`, `memory_dir`, `Element Quality Checks`, `Design System Token Management`, `Browser Antipattern Detection`, `initPageChat`, `Browser Design Analysis`, `Antipattern Detection CLI`, `Static Content Style Analysis`, `Browser Finding Serialization`, `Hero Element & Color Checks`, `Visual Contrast Analysis`, `Static CSS Processing`?**
...
- **Why does `el()` connect `Configuration Bar UI Controls` to `live-browser.js`, `checks.mjs`, `Browser Visual Contrast Analysis`, `parseAnyColor`, `Live Editing State Management`, `resolveLengthPx`, `Schemas`, `design-system.mjs`, `Critique Snapshot Storage`, `Page Chat Voice UI`, `Critique Snapshot Storage`, `collectBrowserFindings`, `detect-antipatterns-browser.js`, `detect-antipatterns.mjs`, `detect-html.mjs`, `Browser Finding Serialization`, `Visual Contrast Analysis`, `css-cascade.mjs`, `UI Toolbar & Themes`?**
